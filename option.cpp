				     ...
				     * see option.h for definitions. */
{
 ...
 optionPtr->key, *curArg, *((double *) optionPtr->address));
			    ...
			    break;
			case OPT_GENFUNC: {
			    int	    (*handlerProc)(const char*, int, char**);

			    handlerProc = (int (*)(const char*, int, char**))optionPtr->address;
			...
			case OPT_FUNC: {
			    int (*handlerProc)(const char*, char*);

			    handlerProc = (int (*)(const char*, char*))optionPtr->address;
